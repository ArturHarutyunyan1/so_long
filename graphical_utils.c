...
void init_textures(t_game *game)
{
    ...
    game->textures.dot = mlx_xpm_file_to_image(game->mlx, "textures/floor_eaten.xpm", &len, &len);
...
void destroy_textures(t_game *game)
{
    ...
    else if (game->textures.dot)
        mlx_destroy_image(game->mlx, game->textures.dot);
...
void draw(t_game *game, int x, int y, char c)
{
    if (c == '0')
        ...
        mlx_put_image_to_window(game->mlx, game->mlx_win, game->textures.dot, x, y);
    else if (c == 'E')
